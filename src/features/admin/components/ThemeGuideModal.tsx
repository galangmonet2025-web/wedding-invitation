...
export function ThemeGuideModal({ isOpen, onClose, previewTenant, imageTypes = [], activeTab, onTabChange }: ThemeGuideModalProps) {
    ...
    const variableGroups = [
        {
            title: '🖼️ Foto & Media Utama',
            vars: [
                ...imageVariables,
                { tag: '{{photo_hero_cover}}', desc: 'URL Foto Sampul Utama', value: 'https://cdn...', type: 'URL String', code: '<img src="{{photo_hero_cover}}" alt="Hero Cover" />' },
                { tag: '{{photo_groom_photo}}', desc: 'URL Foto Pengantin Pria', value: 'https://cdn...', type: 'URL String', code: '<img src="{{photo_groom_photo}}" alt="Groom" />' },
                { tag: '{{photo_bride_photo}}', desc: 'URL Foto Pengantin Wanita', value: 'https://cdn...', type: 'URL String', code: '<img src="{{photo_bride_photo}}" alt="Bride" />' },
                { tag: '{{photo_background}}', desc: 'URL Foto Background Undangan', value: 'https://cdn...', type: 'URL String', code: '<div style="background-image: url({{photo_background}})"></div>' },
                { tag: '{{photo_closing}}', desc: 'URL Foto Closing / Penutup', value: 'https://cdn...', type: 'URL String', code: '<img src="{{photo_closing}}" alt="Closing" />' },
                { tag: '{{photo_story_photo}}', desc: 'URL Foto Kisah Cinta', value: 'https://cdn...', type: 'URL String', code: '<img src="{{photo_story_photo}}" alt="Story" />' },
                ...
                { tag: '{{link_backsound_music}}', desc: 'URL Musik Background', value: 'https://cdn...', type: 'URL String', code: '<audio src="{{link_backsound_music}}" id="bgm" loop></audio>' },
        ...
        },
        {
            ...
            vars: [
                { tag: '{{#if tampilkan_amplop_online}}', desc: 'Kondisi Tampilkan Amplop Digital', value: '(Block Logic)', type: 'Boolean Logic', code: '{{#if tampilkan_amplop_online}}\n  <!-- Info Bank -->\n{{/if}}' },
                { tag: '{{#if flag_pakai_2_rekening}}', desc: 'Kondisi Gunakan 2 Rekening', value: '(Block Logic)', type: 'Boolean Logic', code: '{{#if flag_pakai_2_rekening}}\n  <!-- Rekening 2 -->\n{{/if}}' },
                ...
                { tag: '{{#if flag_pakai_qris_rekening_1}}', desc: 'Kondisi Pakai QRIS Rek 1', value: 'true', type: 'Boolean Logic', code: '{{#if flag_pakai_qris_rekening_1}}\n  <img src="{{gambar_qris_rekening_1}}" />\n{{/if}}' },
                { tag: '{{gambar_qris_rekening_1}}', desc: 'URL Gambar QRIS Rek 1', value: 'https://cdn...', type: 'URL String', code: '<img src="{{gambar_qris_rekening_1}}" />' },
                ...
                { tag: '{{#if flag_pakai_qris_rekening_2}}', desc: 'Kondisi Pakai QRIS Rek 2', value: 'false', type: 'Boolean Logic', code: '{{#if flag_pakai_qris_rekening_2}}\n  <img src="{{gambar_qris_rekening_2}}" />\n{{/if}}' },
                { tag: '{{gambar_qris_rekening_2}}', desc: 'URL Gambar QRIS Rek 2', value: 'https://cdn...', type: 'URL String', code: '<img src="{{gambar_qris_rekening_2}}" />' },
                { tag: '{{#if flag_kirim_hadiah_offline}}', desc: 'Kondisi Hadiah Fisik (Alamat)', value: '(Block Logic)', type: 'Boolean Logic', code: '{{#if flag_kirim_hadiah_offline}}\n  <!-- Render Alamat -->\n{{/if}}' },
        ...
        },
        {
            ...
            vars: [
                ...
                { tag: '{{custom_kalimat_1}}', desc: 'Teks Kustom 1 (Kutipan)', value: 'Teks Tambahan 1', type: 'String', code: '<p>{{custom_kalimat_1}}</p>' },
        ...
        },
        {
            ...
            vars: [
                ...
                { tag: '{{this.guest_message}}', desc: 'Pesan/Ucapan (Dalam wishes)', value: 'Semoga langgeng...', type: 'String', code: '<p>{{this.guest_message}}</p>' },
                { tag: '{{this.guest_comment_time}}', desc: 'Waktu Ucapan', value: '2 jam lalu', type: 'String', code: '<span>{{this.guest_comment_time}}</span>' },
                { tag: '{{this.guest_initial}}', desc: 'Inisial Nama', value: 'B', type: 'String', code: '<div>{{this.guest_initial}}</div>' },
            ]
        },
        {
            title: '⏱️ Countdown (Live)',
            vars: [
                { tag: '{{countdown_hari}}', desc: 'Angka Hari tersisa', value: '12', type: 'Number', code: '<div id="hari">{{countdown_hari}}</div>' },
                { tag: '{{countdown_jam}}', desc: 'Angka Jam tersisa', value: '05', type: 'Number', code: '<div id="jam">{{countdown_jam}}</div>' },
                { tag: '{{countdown_menit}}', desc: 'Angka Menit tersisa', value: '30', type: 'Number', code: '<div id="menit">{{countdown_menit}}</div>' },
                { tag: '{{countdown_detik}}', desc: 'Angka Detik tersisa', value: '45', type: 'Number', code: '<div id="detik">{{countdown_detik}}</div>' },
            ]
        },
        {
            title: '🔄 Fitur & Logic (Flags)',
            vars: [
                { tag: '{{#if flag_pakai_timeline_kisah}}', desc: 'Ada Timeline Kisah Cinta', value: '(Block Logic)', type: 'Boolean Logic', code: '{{#if flag_pakai_timeline_kisah}}\n  <!-- Render Timeline -->\n{{/if}}' },
                { tag: '{{#each timeline_kisah}}', desc: 'Loop Data Timeline', value: '(Block Logic)', type: 'Looping Logic', code: '{{#each timeline_kisah}}\n  <p>{{this.judul}}</p>\n{{/each}}' },
                { tag: '{{this.tanggal}}', desc: 'Tgl (Dalam timeline)', value: 'Januari 2020', type: 'String', code: '<span>{{this.tanggal}}</span>' },
                { tag: '{{this.judul}}', desc: 'Judul (Dalam timeline)', value: 'Pertama Kali Bertemu', type: 'String', code: '<h4>{{this.judul}}</h4>' },
                { tag: '{{this.deskripsi}}', desc: 'Desc (Dalam timeline)', value: 'Kisah awal...', type: 'String', code: '<p>{{this.deskripsi}}</p>' },
                { tag: '{{#if has_gallery}}', desc: 'Punya minimal 1 foto galeri', value: 'true', type: 'Boolean Logic', code: '{{#if has_gallery}}\n  <!-- Section Galeri -->\n{{/if}}' },
                { tag: '{{#if is_fitur_gallery}}', desc: 'Fitur Galeri Aktif (Plan)', value: '(Block Logic)', type: 'Boolean Logic', code: '{{#if is_fitur_gallery}}\n  <!-- Konten Galeri -->\n{{/if}}' },
                { tag: '{{#each galleries}}', desc: 'Loop Image Galeri', value: '(Block Logic)', type: 'Looping Logic', code: '{{#each galleries}}\n  <img src="{{this.url}}" />\n{{/each}}' },
                { tag: '{{this.url}}', desc: 'URL Gambar (Dalam galleries)', value: 'https://...', type: 'URL String', code: '<img src="{{this.url}}">' },
                { tag: '{{#if flag_lokasi_akad_dan_resepsi_berbeda}}', desc: 'Lokasi Akad & Resepsi Berbeda', value: '(Block Logic)', type: 'Boolean Logic', code: '{{#if flag_lokasi_akad_dan_resepsi_berbeda}}\n  <!-- Info Beda Lokasi -->\n{{/if}}' },
            ...
            <div className="py-2 space-y-6 text-sm text-gray-700 dark:text-gray-300 h-[60vh] overflow-y-auto pr-2 custom-scrollbar">
                ...
                {activeTab === 'guide' ? (
                    ...
                    <div className="space-y-6">
                        ...
                        <section className="space-y-4">
                            <h4 className="text-md font-bold text-gray-800 dark:text-white flex items-center gap-2">
                                <span className="text-orange-500">3.</span> Looping Data Array (Each)
                            </h4>
                            <p className="text-sm">Gunakan blok <code>{`{{#each variabel_array}} ... {{/each}}`}</code> untuk mengulang elemen HTML sebanyak jumlah data riil. Di dalam blok iterasi, gunakan <code>{`{{this.field}}`}</code> untuk mengakses isi datanya.</p>
                        </section>
                    </div>
                ) : (
                    <section>
                        <p className="mb-4 text-gray-600 dark:text-gray-400">
                            Berikut adalah daftar variabel (*Tag*) yang dapat Anda gunakan di dalam HTML Code. Tabel di bawah ini dikelompokkan untuk memudahkan pencarian.
                        </p>
                        
                        <div className="space-y-8">
                            {variableGroups.map((group, groupIdx) => (
                                <div key={groupIdx} className="space-y-3">
                                    <h4 className="text-md font-bold text-gold-600 dark:text-gold-400 flex items-center gap-2 border-l-4 border-gold-500 pl-3">
                                        {group.title}
                                    </h4>
                                        ...
                                        <table className="w-full table-fixed text-left text-sm whitespace-normal break-words">
                                            <thead className="bg-gray-50 dark:bg-gray-800 text-gray-700 dark:text-gray-300">
                                                <tr>
                                                    ...
                                                    <th className="px-4 py-3 font-semibold border-b dark:border-gray-700 w-[30%]">Contoh Render</th>
                                                </tr>
                                            </thead>
