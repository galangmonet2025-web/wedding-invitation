/* =========================================================================
 BETAWI HERITAGE — theme JS
 Betawi palette (ivory / antique gold / deep green) with the siangko,
 tumpal (pucuk rebung) motif bands & yellow botanical sprays.
 Structure/host-wiring cloned from the "sunda-heritage" theme (itself
 cloned from "jawa-heritage" → "minang-heritage" → "timeless").
 ...
 - NEVER call audio.play() and NEVER mirror #bg-music state — the host
 owns the real player AND is the only writer of the play/pause icon.
 We just keep the host IDs verbatim and let it intercept
 #btn-toggle-music (see the MUSIC ICON note further down).
 ========================================================================= */
(function () {
 'use strict';

 // ---- Run previous instance's cleanup, then start a fresh registry -------
 if (typeof window.__betawiCleanup === 'function') {
 try { window.__betawiCleanup(); } catch (e) { /* noop */ }
 }
 var cleanupFns = [];
 window.__betawiCleanup = function () {
 ...
 // =====================================================================
 // Countdown — reads the REAL wedding date from the DB (#bw-wed-date).
 // Self-contained + guards against stacked intervals on re-execution.
 // =====================================================================
 (function startCountdown() {
 if (window.__bwCountdownTimer) {
 clearInterval(window.__bwCountdownTimer);
 window.__bwCountdownTimer = null;
 }

 function resolveDay() {
 var holder = document.getElementById('wedding-calendar')
 || document.getElementById('bw-wed-date');
 ...
 var holder = document.getElementById('bw-jam-resepsi');
 ...
 clearInterval(window.__bwCountdownTimer); window.__bwCountdownTimer = null;
 } else {
 ...
 tick();
 window.__bwCountdownTimer = setInterval(tick, 1000);
 cleanupFns.push(function () {
 if (window.__bwCountdownTimer) { clearInterval(window.__bwCountdownTimer); window.__bwCountdownTimer = null; }
 });
 })();
