/* =========================================================================
 DAYAK HERITAGE — theme JS
 Dayak palette (ivory / charcoal / antique gold) with the mahkota bulu
 enggang, ukir motif bands & lili botanicals. Structure/host-wiring cloned
 from the "sunda-heritage" theme (itself descended from "timeless").
...
(function () {
 ...
 if (typeof window.__dayakCleanup === 'function') {
 try { window.__dayakCleanup(); } catch (e) { /* noop */ }
 }
 var cleanupFns = [];
 window.__dayakCleanup = function () {
 ...
 // =====================================================================
 // Countdown — reads the REAL wedding date from the DB (#dy-wed-date).
 // Self-contained + guards against stacked intervals on re-execution.
 // =====================================================================
 (function startCountdown() {
 if (window.__dyCountdownTimer) {
 clearInterval(window.__dyCountdownTimer);
 window.__dyCountdownTimer = null;
 }

 function resolveDay() {
 var holder = document.getElementById('wedding-calendar')
 || document.getElementById('dy-wed-date');
 ...
 var holder = document.getElementById('dy-jam-resepsi');
 ...
 clearInterval(window.__dyCountdownTimer); window.__dyCountdownTimer = null;
 } else {
 ...
 tick();
 window.__dyCountdownTimer = setInterval(tick, 1000);
 cleanupFns.push(function () {
 if (window.__dyCountdownTimer) { clearInterval(window.__dyCountdownTimer); window.__dyCountdownTimer = null; }
 });
 })();
