...
(function () {
    ...
    function openPieceModal(key) {
        ...
        pauseGame();   // freeze the run while the guest reads a piece — no dying behind the modal
    }
    function closeModal() {
        ...
        resumeGame();  // un-freeze on close (no-op if a reveal/other view is still up)
    }
    ...
    function closeReveal() {
        ...
        try {
            if (window.__mswStarted) {
                var sc = scene();
                // Re-boot when the game is gone OR its canvas was DETACHED by a host re-injection
                // (GAME/sc still exist but render to an orphaned canvas → blank). startRun() tears
                // the stale detached game down and boots cleanly into the fresh #gw-stage.
                if (!GAME || !sc || !gameStageAttached()) {
    ...
    function init() {
        ...
        try { startSideHeal(); } catch (e) {}   // keep the decorative couple canvas painted across host re-injections
    ...
    /* SELF-HEAL the decorative side canvas across host re-injections.
       On the DEPLOYED invitation the host re-injects the theme HTML (dangerouslySetInnerHTML)
       on any image-resolve / guest-state change WITHOUT re-running the theme JS. That swaps in a
       FRESH, blank #msw-couple-canvas element (a <canvas>'s pixels are wiped when the node is
       replaced) — and drawCoupleCanvas() never runs again, so the desktop right-panel tableau
       shows an EMPTY black box ("gambar muncul di awal, blink, lalu hilang"). We mark the canvas
       with data-mswPainted after drawing; this lightweight RAF watchdog notices when a re-injected
       canvas has lost that mark (or is a different node) and re-draws it + re-applies the side bg.
       Throttled to ~4×/sec so it's cheap; stops on cleanup. */
    var _sideHealRAF = 0, _sideHealLast = 0;
    function startSideHeal() {
        stopSideHeal();
        var tick = function (t) {
            _sideHealRAF = window.requestAnimationFrame(tick);
            if (t - _sideHealLast < 250) return;   // ~4 checks/sec
            _sideHealLast = t;
            try {
                var cv = $('msw-couple-canvas');
                if (cv && cv.getContext && cv.dataset.mswPainted !== '1') {
                    drawCoupleCanvas();   // re-injected blank canvas → repaint (re-sets the mark)
                    paintSideBg();
                }
                // version badge: a re-injection resets #msw-version to the static "v0.0.0"
                // placeholder (JS doesn't re-run), so re-stamp it here too.
                var vb = $('msw-version');
                if (vb && vb.textContent !== VERSION) vb.textContent = VERSION;
            } catch (e) {}
        };
        _sideHealRAF = window.requestAnimationFrame(tick);
        onCleanup(stopSideHeal);
    }
    function stopSideHeal() {
        if (_sideHealRAF) { try { window.cancelAnimationFrame(_sideHealRAF); } catch (e) {} _sideHealRAF = 0; }
    }
    ...
    function drawCoupleCanvas() {
        ...
        x.clearRect(0, 0, W, H);
        try { cv.dataset.mswPainted = '1'; } catch (e) {}   // mark so the heal loop knows this canvas is drawn
    ...
    function startRun(sector) {
        ...
        // If a host RE-INJECTION swapped out #gw-stage, the live GAME now renders to a DETACHED
        // canvas — hot-loading into it would show nothing (dead START after the "blink"). Tear the
        // stale game down so the branch below falls through to a clean re-boot into the fresh stage.
        if (GAME && !gameStageAttached()) {
            try { GAME.destroy(true); } catch (e) {}
            GAME = null; window.__gwGame = null;
        }
        var sc = scene();
        if (GAME && sc && sc.loadSector && gameStageAttached()) {
    ...
    // Is the live game's canvas STILL in the current DOM? On the deployed invitation the host
    // re-injects the theme HTML (dangerouslySetInnerHTML) on any guest-state / image-resolve
    // change WITHOUT re-running the theme JS, so #gw-stage (and Phaser's <canvas> inside it) is
    // replaced by fresh source markup while the JS `GAME` object keeps pointing at the OLD,
    // now-DETACHED canvas. Hot-loading a sector into that detached game renders nothing → the
    // cover's START button looks dead after the "blink". This returns false in exactly that
    // state so startRun() can force a clean re-boot into the fresh #gw-stage instead.
    function gameStageAttached() {
        try {
            if (!GAME || !GAME.canvas) return false;
            // canvas must be connected to the live document AND sit inside the CURRENT #gw-stage
            var stage = $('gw-stage');
            if (!stage) return false;
            if (!document.body.contains(GAME.canvas)) return false;   // detached by re-injection
            if (!stage.contains(GAME.canvas)) return false;           // canvas belongs to an old stage
            return true;
    ...
    defineAndBoot = function () {
        ...
        var config = {
            ...
            /* FIXED-STEP PHYSICS (fix "loncat tinggi beda PC vs mobile / naik plafon di PC").
               Without fps+fixedStep, Arcade integrates with the RAW frame delta, so semi-implicit
               Euler produces a DIFFERENT jump apex per refresh rate: a 120/144Hz PC (small dt)
               overshoots (~naik plafon), a 60Hz/throttled phone (big dt) undershoots (loncat
               pendek). The whole level-gen assumes the "@60fps" apex (~100px), so we PIN the sim
               to a constant 60Hz step on every device → identical apex everywhere. fpsLimit caps
               the render loop so a 120/144Hz PC no longer runs extra sub-steps either. */
            fps: { target: 60, forceSetTimeOut: false, min: 30, limit: 60 },
            physics: { default: 'arcade', arcade: { gravity: { y: CONFIG.player.gravity }, fps: 60, fixedStep: true, debug: false } },
            scene: [GameScene]
        };
