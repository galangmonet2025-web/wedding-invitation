...
(function () {
    ...
    if (typeof window.__bpassCleanup === 'function') {
        try { window.__bpassCleanup(); } catch (e) { /* noop */ }
    }
    var cleanupFns = [];
    window.__bpassCleanup = function () {
    ...
    (function startCountdown() {
        if (window.__bpCountdownTimer) {
            clearInterval(window.__bpCountdownTimer);
            window.__bpCountdownTimer = null;
        }
        ...
        function tick() {
            ...
            if (dist > 0) {
                ...
                setStatus('Penerbangan menuju hari bahagia telah tiba ✈️');
                clearInterval(window.__bpCountdownTimer); window.__bpCountdownTimer = null;
            } else {
                setStatus('Perjalanan kami telah mendarat. Terima kasih atas doa & restunya 🙏');
                clearInterval(window.__bpCountdownTimer); window.__bpCountdownTimer = null;
            }
        }
        tick();
        window.__bpCountdownTimer = setInterval(tick, 1000);
        cleanupFns.push(function () {
            if (window.__bpCountdownTimer) { clearInterval(window.__bpCountdownTimer); window.__bpCountdownTimer = null; }
        });
    })();
    ...
    // listeners survives host HTML re-injection.
    // =====================================================================
    ...
    // =====================================================================
    // Share-happiness story carousel (horizontal snap). Re-queries its own
    // nodes each run and registers cleanup so listeners never stack.
    // =====================================================================
