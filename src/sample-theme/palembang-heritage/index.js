/* =========================================================================
 PALEMBANG HERITAGE — theme JS
 Palembang palette (ivory / deep red / gold) with the aesan gede crown,
 songket motif bands & red botanical sprays. Structure/host-wiring cloned
 from the "sunda-heritage" theme (a lineage going back to "timeless").
 ...
 we just let the host intercept #btn-toggle-music (see the MUSIC ICON
 note further down for why mirroring breaks the icon).
 ========================================================================= */
(function () {
 'use strict';

 // ---- Run previous instance's cleanup, then start a fresh registry -------
 if (typeof window.__palembangCleanup === 'function') {
 try { window.__palembangCleanup(); } catch (e) { /* noop */ }
 }
 var cleanupFns = [];
 window.__palembangCleanup = function () {
 ...
 // =====================================================================
 // Countdown — reads the REAL wedding date from the DB (#pl-wed-date).
 // Self-contained + guards against stacked intervals on re-execution.
 // =====================================================================
 (function startCountdown() {
 if (window.__plCountdownTimer) {
 clearInterval(window.__plCountdownTimer);
 window.__plCountdownTimer = null;
 }

 function resolveDay() {
 var holder = document.getElementById('wedding-calendar')
 || document.getElementById('pl-wed-date');
 ...
 var holder = document.getElementById('pl-jam-resepsi');
 ...
 clearInterval(window.__plCountdownTimer); window.__plCountdownTimer = null;
 } else {
 ...
 tick();
 window.__plCountdownTimer = setInterval(tick, 1000);
 cleanupFns.push(function () {
 if (window.__plCountdownTimer) { clearInterval(window.__plCountdownTimer); window.__plCountdownTimer = null; }
 });
 })();
