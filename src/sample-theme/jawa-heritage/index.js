/* =========================================================================
   JAWA HERITAGE — theme JS
   Javanese palette (ivory / gold / sogan brown) with the wayang gunungan,
   kawung batik bands & lily botanicals. Structure/host-wiring cloned from
...
(function () {
    ...
    if (typeof window.__jawaCleanup === 'function') {
        try { window.__jawaCleanup(); } catch (e) { /* noop */ }
    }
    var cleanupFns = [];
    window.__jawaCleanup = function () {
    ...
    // =====================================================================
    // Countdown — reads the REAL wedding date from the DB (#jw-wed-date).
    // Self-contained + guards against stacked intervals on re-execution.
    // =====================================================================
    (function startCountdown() {
        if (window.__jwCountdownTimer) {
            clearInterval(window.__jwCountdownTimer);
            window.__jwCountdownTimer = null;
        }

        function resolveDay() {
            var holder = document.getElementById('wedding-calendar')
                || document.getElementById('jw-wed-date');
        ...
        function receptionEnd(day) {
            var holder = document.getElementById('jw-jam-resepsi');
        ...
        function tick() {
            ...
            if (dist > 0) {
                ...
                clearInterval(window.__jwCountdownTimer); window.__jwCountdownTimer = null;
            } else {
        ...
        tick();
        window.__jwCountdownTimer = setInterval(tick, 1000);
        cleanupFns.push(function () {
            if (window.__jwCountdownTimer) { clearInterval(window.__jwCountdownTimer); window.__jwCountdownTimer = null; }
        });
    })();
