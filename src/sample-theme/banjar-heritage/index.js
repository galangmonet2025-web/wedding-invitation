/* =========================================================================
 BANJAR HERITAGE — theme JS
 Banjarese palette (ivory / deep maroon / antique gold) with the mahkota
 gajah gamuling, sasirangan motif bands & floral botanicals. Structure and
 host-wiring cloned from the "sunda-heritage" theme.
 ...
 owns the real player AND is the only writer of the play/pause icon; we
 just keep the host IDs so it can intercept #btn-toggle-music.
 ========================================================================= */
(function () {
 'use strict';

 // ---- Run previous instance's cleanup, then start a fresh registry -------
 if (typeof window.__banjarCleanup === 'function') {
 try { window.__banjarCleanup(); } catch (e) { /* noop */ }
 }
 var cleanupFns = [];
 window.__banjarCleanup = function () {
 ...
 // =====================================================================
 // Countdown — reads the REAL wedding date from the DB (#bj-wed-date).
 // Self-contained + guards against stacked intervals on re-execution.
 // =====================================================================
 (function startCountdown() {
 if (window.__bjCountdownTimer) {
 clearInterval(window.__bjCountdownTimer);
 window.__bjCountdownTimer = null;
 }

 function resolveDay() {
 var holder = document.getElementById('wedding-calendar')
 || document.getElementById('bj-wed-date');
 ...
 var holder = document.getElementById('bj-jam-resepsi');
 ...
 clearInterval(window.__bjCountdownTimer); window.__bjCountdownTimer = null;
 } else {
 ...
 tick();
 window.__bjCountdownTimer = setInterval(tick, 1000);
 cleanupFns.push(function () {
 if (window.__bjCountdownTimer) { clearInterval(window.__bjCountdownTimer); window.__bjCountdownTimer = null; }
 });
 })();
