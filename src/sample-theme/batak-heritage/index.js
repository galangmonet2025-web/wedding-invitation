/* =========================================================================
 BATAK HERITAGE — theme JS
 Batak palette (ivory / deep red / gold) with the rumah bolon gable,
 ulos motif bands & rose-fern botanicals. Structure/host-wiring cloned from
 the "sunda-heritage" theme (a descendant of "timeless").
 ...
 we just let the host intercept #btn-toggle-music.
 ========================================================================= */
(function () {
 'use strict';

 // ---- Run previous instance's cleanup, then start a fresh registry -------
 if (typeof window.__batakCleanup === 'function') {
 try { window.__batakCleanup(); } catch (e) { /* noop */ }
 }
 var cleanupFns = [];
 window.__batakCleanup = function () {
 ...
 // =====================================================================
 // Countdown — reads the REAL wedding date from the DB (#bt-wed-date).
 // Self-contained + guards against stacked intervals on re-execution.
 // =====================================================================
 (function startCountdown() {
 if (window.__btCountdownTimer) {
 clearInterval(window.__btCountdownTimer);
 window.__btCountdownTimer = null;
 }

 function resolveDay() {
 var holder = document.getElementById('wedding-calendar')
 || document.getElementById('bt-wed-date');
 ...
 var holder = document.getElementById('bt-jam-resepsi');
 ...
 clearInterval(window.__btCountdownTimer); window.__btCountdownTimer = null;
 } else {
 ...
 tick();
 window.__btCountdownTimer = setInterval(tick, 1000);
 cleanupFns.push(function () {
 if (window.__btCountdownTimer) { clearInterval(window.__btCountdownTimer); window.__btCountdownTimer = null; }
 });
 })();
