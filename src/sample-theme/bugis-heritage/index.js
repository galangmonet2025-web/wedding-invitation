/* =========================================================================
 BUGIS HERITAGE — theme JS
 Bugis palette (cream / deep green / antique gold) with the mahkota +
 songkok recca', tumpal motif bands & rose-fern botanicals. Structure/
 host-wiring cloned from the "sunda-heritage" theme (itself cloned from
 "jawa-heritage" → "minang-heritage" → "timeless").
...
(function () {
 ...
 if (typeof window.__bugisCleanup === 'function') {
 try { window.__bugisCleanup(); } catch (e) { /* noop */ }
 }
 var cleanupFns = [];
 window.__bugisCleanup = function () {
 ...
 // =====================================================================
 // Countdown — reads the REAL wedding date from the DB (#bu-wed-date).
 // Self-contained + guards against stacked intervals on re-execution.
 // =====================================================================
 (function startCountdown() {
 if (window.__buCountdownTimer) {
 clearInterval(window.__buCountdownTimer);
 window.__buCountdownTimer = null;
 }

 function resolveDay() {
 var holder = document.getElementById('wedding-calendar')
 || document.getElementById('bu-wed-date');
 ...
 var holder = document.getElementById('bu-jam-resepsi');
 ...
 clearInterval(window.__buCountdownTimer); window.__buCountdownTimer = null;
 } else {
 ...
 tick();
 window.__buCountdownTimer = setInterval(tick, 1000);
 cleanupFns.push(function () {
 if (window.__buCountdownTimer) { clearInterval(window.__buCountdownTimer); window.__buCountdownTimer = null; }
 });
 })();
